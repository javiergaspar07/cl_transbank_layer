...
class TransbankController(object):
    ...
    def __init__(self, integration_type):
        ...
        Fields:
            ...
            INTEGRATION_TYPE (int): Transbank integration type.
...
class TransbankCheckoutRequest(BaseModel):
    ...
    - product_id (ProductType): Code to identify a product
    - amount (int): Total amount to pay
    - session_id (str): Code generated to identify a session in Transbank API
    - return_url (AnyUrl): URL to redirect the client once the payment
    has been made.
    """
    buy_order: str = uuid()
